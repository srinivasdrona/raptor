...
| **(c) Expert validator** | Mechanical checks (schema/hash/state/conservation/redaction) validate *form*; a QMG validates any externally meaningful disposition. This increment authorizes the **batch build**, never a release (STRATEGY §9 two-sign-off). |
